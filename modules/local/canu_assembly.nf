...
process assembleCore_canu {
    ...
    python3 ${workflow.projectDir}/bin/filter_trycycler_contigs.py \
        --approx_size ${meta.approx_size} \
        --cluster_dir $cluster_dir \
        --phylip trycycler/contigs.phylip \
        --max_keep 26 &&
    
    # 3) Reconcile (now should not hit >26 or huge length outliers)
    STATUS="Failed to reconcile assemblies" &&
    (trycycler reconcile \
        --reads "${meta.alias}.downsampled.fastq" \
        --cluster_dir $cluster_dir \
    ...
    # 4) Gate: do not proceed unless reconcile actually produced its output
    test -s $cluster_dir/2_all_seqs.fasta &&
    
    # 5) MSA / partition / consensus
    ...
    if [ ! -f "${cluster_dir}/7_final_consensus.fasta" ]; then
        if ls ${cluster_dir}/1_contigs/*.fasta 1> /dev/null 2>&1; then
            ...
            (seqkit sort ${cluster_dir}/1_contigs/*.fasta --by-length -r \
